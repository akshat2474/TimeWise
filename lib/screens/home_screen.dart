...
class _HomeScreenState extends State<HomeScreen> {
  ...
  SliverAppBar _buildSliverAppBar(BuildContext context, ThemeData theme, bool hasExistingTimetable) {
    return SliverAppBar(
      pinned: true,
      stretch: true,
      ...
      actions: [
        if (hasExistingTimetable)
          IconButton(
            ...
            // MODIFIED: Set the icon color to white
            color: Colors.white,
          ),
      ],
    );
  ...
  Widget _buildEmptyStateView(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        Icon(Icons.calendar_today_outlined, size: 48, color: theme.colorScheme.primary),
        const SizedBox(height: 24),
        Text(
          'Welcome to TimeWise!',
        ...
        ElevatedButton.icon(
          onPressed: () {
            // TODO: Navigate to your create/edit timetable screen
            // Navigator.pushNamed(context, '/create-timetable');
          },
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (nextClass != null) ...[
                      ...
                      Text("No more classes!", style: theme.textTheme.bodyMedium),
                  ],
                ),
              ),
            ),
              ...
              child: _buildStyledCard(
                child: Column(
                  ...
                  children: [
                    ...
                    Text("Overall", style: theme.textTheme.bodyMedium?.copyWith(color: Colors.white70)),
                  ],
                ),
              ),
            ),
              ...
              : ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    if (timeSlotParts.length == 2) {
                      try {
                        startTime = TimeOfDay(hour: int.parse(timeSlotParts[0].split(':')[0]), minute: int.parse(timeSlotParts[0].split(':')[1]));
                        endTime = TimeOfDay(hour: int.parse(timeSlotParts[1].split(':')[0]), minute: int.parse(timeSlotParts[1].split(':')[1]));
                      } catch (e) {
                    ...
                    final double startInMinutes = startTime != null ? startTime.hour * 60.0 + startTime.minute : -1;
                    final double endInMinutes = endTime != null ? endTime.hour * 60.0 + endTime.minute : -1;

                    bool isLive = nowInMinutes >= startInMinutes && nowInMinutes < endInMinutes;
                      ...
                      children: [
                        ...
                        if (isLive) ...[
                          Container(
                            ...
                            child: Text(
                              "LIVE",
                              style: theme.textTheme.labelSmall?.copyWith(color: AppTheme.secondary, fontWeight: FontWeight.bold),
                            ),
                          ),
                          const SizedBox(width: 12),
                          Text(
